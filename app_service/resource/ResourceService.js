...
import CategoryCacheService from '../category/CategoryCacheService';
import TagCacheService from '../tag/TagCacheService';
import UserCacheService from '../user/UserCacheService';

const log = log4js.getLogger('ResourceService');
...
class ResourceService {
  ...
  async retrieveAllResource(req) {
    try {
      ...
      const results = await this.ResourceDao.findResourcePages(
        query,
        sort,
        start,
        pageSize,
      ...
      for (let i = 0; i < results.length; i++) {
        const categoryLa = await this.CategoryCacheService.getCategory(
          results[i].categoryLaId,
        );
        results[i].categoryLaName = categoryLa ? categoryLa.name : '';
        results[i].tags = [];
        for (const j of results[i].tagId) {
        ...
        results[i].dateCreated = moment(results[i].dateCreated).format(
          'YYYY-MM-DD HH:mm:ss',
        );
        results[i].dateModified = moment(results[i].dateModified).format(
          'YYYY-MM-DD HH:mm:ss',
        );
      }
      return {
